---
name: rag-reindex
description: docs-rag-mcp 문서 색인을 재생성하고 검증한다. 코퍼스 문서를 추가·수정·삭제한 뒤, 또는 search_docs 결과가 낡아 보일 때 호출한다. 색인 신선도 확인 → 재색인 → 왕복 검증 → 회귀 게이트 → 새 문서 자기검색 스모크까지 한 흐름으로 돌리고 결과를 요약한다. 문서 작성 자체는 하지 않는다(운영 전용).
---
